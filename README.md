...
During a chat session the agent automatically selects and invokes tools to fulfil your request. You can also trigger specific tools by describing what you want naturally:

```
You: read the file /tmp/notes.txt
Agent: [calls filesystem.read_file] Here are the contents of notes.txt: ...

You: list all Python files in this project recursively
Agent: [calls filesystem.list_directory] Found 42 .py files: ...

You: run `pytest tests/ -q` and show me the results
Agent: [calls shell.execute] All 38 tests passed in 2.4s.

You: fetch https://api.github.com/repos/AI-Alon/laios and summarise it
Agent: [calls web.fetch] The repo has 312 stars, last pushed 2 hours ago ...

You: what changed in the last 5 commits?
Agent: [calls git.log] Here are the last 5 commits: ...
```


## Using Tools Programmatically

You can invoke any registered tool directly from Python without going through the agent:
...
result = registry.execute("filesystem.read_file", {"path": "/tmp/notes.txt"})
print(result.data)          # file contents
print(result.success)       # True / False
print(result.error)         # error message if success=False

# List a directory with a glob pattern
result = registry.execute("filesystem.list_directory", {
    "path": ".",
    "pattern": "*.py",
    "recursive": True,
})
...
result = registry.execute("shell.execute", {
...
# Fetch a URL
result = registry.execute("web.fetch", {
    "url": "https://api.github.com/repos/AI-Alon/laios",
    "method": "GET",
    "headers": {"Accept": "application/vnd.github+json"},
})
repo_info = result.data  # parsed JSON response

# Parse and transform data
result = registry.execute("data.parse_json", {
    "data": '[{"name":"Alice","score":95},{"name":"Bob","score":80}]',
})
result = registry.execute("data.transform", {
    "data": result.data,
    "sort_key": "score",
    "sort_reverse": True,
    "limit": 1,
})
print(result.data)  # [{"name": "Alice", "score": 95}]
```
